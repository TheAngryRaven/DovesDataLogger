...
- **Auto-race** (`autoRaceModeCheck()`): from main menu, if RPM > 500 or
  ...
  it, exiting any page while moving — the course creator especially, since it
  is used out on the course — landed on the menu and entered race mode on the
  very next loop iteration (~4 ms), so the menu was never drawn.
