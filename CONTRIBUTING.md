...
| `BIRDSEYE_ENABLE_SENSOREGG` | `0` | `beta.yml`, and `compile-sketch.yml` for PRs targeting `BETA` | Compiles in the wireless-EGT POC: passive BLE scanner, Temp1 race page, and BLE core up at boot. Off, the accessors return NaN AND (since plan 0013) the `Temp1`/`Junction1`/`Temp2` DOVEX columns, the `temp1_alert_c` setting and the `egt` status-LED mode are compiled out — so a stock log has 13 data columns where a beta log has 16. Readers key off the CSV header line, where all three are optional. |
| `BIRDSEYE_ENABLE_NEOPIXEL` | **`1`** | every channel (the `project.h` default — no workflow passes it) | Compiles in the NeoPixel strip subsystem (plan 0006): on first boot it programs `UICR->NFCPINS` to convert the NFC pads to GPIO (**one-way** — undoing it needs a full chip erase) and self-resets once, then drives pin 30 as the 5 V boost EN and pin 31 as WS2812 data. **The only flag defaulting to 1**, since 4.1.0 — that conversion is therefore charged to every device in the field, LEDs wired or not. Forcing `=0` compiles it out; on a board already converted the stubs still hold boost EN low. Needs the `Adafruit NeoPixel` library. |
| `BIRDSEYE_ENABLE_PROFILING` | `0` | `beta.yml`, and `compile-sketch.yml` for PRs targeting `BETA` | Compiles in main-loop CPU profiling (plan 0011): every subsystem call in `loop()` is timed, rolled up once a second onto a **LOOP PROFILE** race page, and pin 30 is driven as a scope-readable profiling output. **It takes pin 30 away from the NeoPixel boost converter's EN line**, so a profiling build can never switch the 5 V rail — the regulator stays at its hardware default (on) even through System OFF, where the level is retained. A profiling unit left asleep on a battery with LEDs wired will drain it, and an EN jumper left connected gets chopped at loop rate. Bench builds only. Set `-DPROFILING_PIN_SECTION=PROF_SEC_<X>` to point the pin at one subsystem instead of the whole iteration. |
