...
Rgb hueToRgb(uint8_t hue) {
  ...
  if (hue < 85) {
    uint8_t ramp = (uint8_t)(hue * 3);
    return Rgb{(uint8_t)(255 - ramp), ramp, 0};
  }
  if (hue < 170) {
    uint8_t ramp = (uint8_t)((hue - 85) * 3);
    return Rgb{0, (uint8_t)(255 - ramp), ramp};
  }
  uint8_t ramp = (uint8_t)((hue - 170) * 3);
...
static void addSparkles(Frame& f, uint32_t tMs, uint32_t seed,
                        uint8_t chance) {
  uint32_t slot = tMs / kSparkleSlotMs;
  uint32_t phase = tMs % kSparkleSlotMs;
  // Triangle envelope 0..255..0 across the slot.
  uint32_t half = kSparkleSlotMs / 2;
  uint32_t env = phase < half ? (phase * 255) / half
                              : ((kSparkleSlotMs - phase) * 255) / half;
  for (int i = 0; i < kPixelCount; i++) {
    uint32_t h = mix(seed ^ (slot * (uint32_t)kPixelCount + (uint32_t)i));
    if ((h & 0xFFU) < chance) {
      // Peak brightness varies per glint (128..255).
      uint8_t peak = (uint8_t)(128 + ((h >> 8) & 0x7FU));
      uint8_t level = (uint8_t)((env * peak) / 255);
...
bool renderBoot(uint32_t tMs, uint32_t seed, Frame& out) {
  ...
  if (fade > 0) {
    ...
    uint32_t headPx = (headPos256 / 256U) % (uint32_t)kPixelCount;
    // Hue advances a full wheel per revolution.
    uint8_t headHue = (uint8_t)(headPos256 / kPixelCount);
    for (int back = 0; back < kBootCometTail; back++) {
      int px = (int)((headPx + (uint32_t)(kPixelCount - back)) %
                     (uint32_t)kPixelCount);
      // Quadratic tail falloff: bright head, fast decay.
      uint32_t lin = (uint32_t)(kBootCometTail - back) * 255U /
                     (uint32_t)kBootCometTail;
      uint8_t level = (uint8_t)((lin * lin) / 255U);
      Rgb c = hueToRgb((uint8_t)(headHue + (uint8_t)(back * 12)));
...
bool renderPurple(uint32_t tMs, uint32_t seed, Frame& out) {
  ...
  if (tMs < kPurpleWaveMs) {
    ...
    int radius = (int)((tMs * (uint32_t)(maxRadius + 1)) / kPurpleWaveMs);
    for (int i = 0; i < kPixelCount; i++) {
